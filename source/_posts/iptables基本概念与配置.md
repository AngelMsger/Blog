...
categories:
- Linux & DevOps
thumbnail: "/images/iptables基本概念与配置.jpg"
---
...
{% codeblock "将默认策略设置为ACCEPT，但在规则尾部将除其他规则匹配外的数据包拒绝" %}
...
{% codeblock "指定-F参数以清空原有规则" %}
# iptables -F INPUT
{% endcodeblock %}
-I参数可以帮助我们在链首部插入一条规则，-s参数表示源地址，-j参数表示符合规则时执行的动作。添加指令，丢弃来自192.168.151.198的数据包：
{% codeblock "添加指令，丢弃来自192.168.151.198的数据包" %}
# iptables -t filter -I INPUT -s 192.168.151.198 -j DROP
{% endcodeblock %}
...
{% codeblock "指定-D参数以删除filter表中INPUT链上编号为5的规则" %}
...
{% codeblock "指定-P参数以将FORWARD链的默认执行动作改为DROP" %}
# iptables -t filter -P FORWARD DROP
{% endcodeblock %}
我们也可以基于协议进行匹配，拒绝来自192.168.151.198，目标为192.168.151.199，且协议为TCP的报文：
{% codeblock "拒绝来自192.168.151.198，目标为192.168.151.199，且协议为TCP的报文" %}
# iptables -I INPUT -s 192.168.151.198 -d 192.168.151.199 -p tcp -j REJECT
{% endcodeblock %}
...
{% codeblock "拒绝来自网卡eth1，且协议为ICMP的包" %}
# iptables -I INPUT -i eth1 -p icmp PREROUTING -j REJECT
{% endcodeblock %}
...
{% codeblock "利用iptables-save来保存配置" %}
...
{% codeblock "将目标为80端口且协议为TCP的数据包转发至8080端口" %}
...
# iptables -N WEB_IN
...
# iptables -I INPUT -p tcp -m multiport --dports 80,443 -j WEB_IN
{% endcodeblock %}

# 总结
本篇介绍了防火墙的主要作用，iptables中5条链4张表的基本概念，规则的查询与修改，这些都是iptables的基本用法。但iptables的作用远不止于此，下篇文章我们介绍iptables拓展模块的使用。
